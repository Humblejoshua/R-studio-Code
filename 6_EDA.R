...
setwd("/Users/keksmacbookair/Desktop/course works/Intro to Data science/data sets")
...
##3. Find the outliers
#if outliers are more thsn 30% we impute the data set according to variable type
...
#make sure the columns are continous variables
...
ggplot(data = Data, mapping = aes(x = Year, y = Revenue)) + 
  ...
  labs(x="Year", 
       y="Revenue",
       title="Bike Sales Revenue Per Year")
#Very weird cause of the outliers so we remove outliers to make observations easier
...
#Using visuals (frequency line graph)
